...
describe("delegated-routing-v1-http-api-client", () => {
  ...
  //   console.log("Retrieved providers:", provs);
  ...
  it("should find providers", async () => {
    ...
    console.log("Sending providers:", JSON.stringify(providers, null, 2));
    ...
    console.log("Add providers response:", {
      status: response.status,
      statusText: response.statusText,
      body: await response.text(),
    });

    // Add delay to ensure server processes the request
    await new Promise((resolve) => setTimeout(resolve, 100));

    const provs = await all(client.getProviders(cid));
    console.log("Retrieved providers:", JSON.stringify(provs, null, 2));
  ...
  it("should handle different Content-Type headers for JSON responses", async () => {
    ...
    for (const contentType of contentTypes) {
      ...
      const provs = await all(client.getProviders(cid));
      console.log(
        `Retrieved providers for ${contentType}:`,
